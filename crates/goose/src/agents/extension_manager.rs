...
impl ExtensionManager {
    ...
    async fn read_resource_from_extension(
        ...
        let read_result = client_guard.read_resource(uri).await.map_err(|_| {
            ErrorData::new(ErrorCode::INTERNAL_ERROR, format!("Could not read resource with uri: {}", uri), None)
        })?;
...
mod tests {
    ...
    async fn test_dispatch_tool_call() {
        ...
        assert!(matches!(
            result.err().unwrap(),
            ErrorData::new(ErrorCode::INTERNAL_ERROR, _, None)
        ));
        ...
        if let Err(err) = result {
            ...
            assert!(matches!(tool_err, ErrorData::new(ErrorCode::INVALID_REQUEST, _, None)));
        } else {
